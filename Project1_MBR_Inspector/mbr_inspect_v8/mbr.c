...
void check_partition_overlap(const MBR* mbr) {
  printf("\n--- Partition Overlap Check ---\n");

  for(int i=0; i<4; i++) {
    const PartitionEntry* pe = &mbr->partitions[i];
    if(pe->partition_type == 0) continue;

    uint8_t pe_start = pe->start_lba;
    uint8_t pe_end = pe->start_lba + pe->num_sectors - 1;
    for(int j=i+1; j<4; j++) {
      const PartitionEntry* other_pe = &mbr->partitions[j];
      if(other_pe->partition_type == 0) continue;

      uint8_t other_pe_start = other_pe->start_lba;
      uint8_t other_pe_end = other_pe->start_lba + other_pe->num_sectors - 1;

      if(!(pe_end < other_pe_start || pe_start > other_pe_end)) {
        printf("Partitions %d(%u-%u) and %d(%u-%u) overlap!\n", i, pe_start, pe_end, j, other_pe_start, other_pe_end);
      } else {
	printf("Partitions overlap check - PASSED\n");
      }
    }
  }
}
